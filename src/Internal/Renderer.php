...
final class Renderer implements RendererInterface
{
    ...
    private function highlightText(string $text): string
    {
        ...
        $text = preg_replace('|\\</code\\>$|', '', (string) $text, 1);  // remove suffix 1
        $text = preg_replace('|\\</span\\>$|', '', (string) $text, 1);  // remove suffix 2
        $text = preg_replace('|^(\\<span style\\="color\\: #[a-fA-F0-9]{0,6}"\\>)(&lt;\\?php&nbsp;)(.*?)(\\</span\\>)|', '$1$3$4', (string) $text);  // remove custom added "<?php "
